...
function getClass(score: number, answered: number) {
  if (answered === 0) return null;
  if (score >= 80) return { label: 'Classe A — Apto', bg: '#E9F7EE', color: '#1A6B30' };
  if (score >= 50) return { label: 'Classe B — Condicionalmente Apto', bg: '#FFF8E1', color: '#8A6400' };
  return { label: 'Classe C — Inapto', bg: '#FEE8E8', color: '#A32D2D' };
}
...
export function StepAutoavaliacao({ formData, onChange, errors }: Props) {
  ...
  const cls = getClass(score, answered);
  const pct = Math.round((answered / 12) * 100);

  return (
    <div className="space-y-5">
      <div>
        <h3 className="font-bold uppercase tracking-tight mb-1" style={{ fontSize: "15px", color: "var(--fb-blue)" }}>Autoavaliação de Qualidade</h3>
        <p className="text-xs" style={{ color: "var(--fb-slate-gray)" }}>Avalie sua empresa em 12 critérios — notas: 0 · 4 · 8 · 10</p>
      </div>
      ...
      <div className="p-4 rounded-md" style={{ background: "var(--fb-light-gray)", border: "1px solid var(--fb-mid-gray)" }}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <TrendingUp className="w-4 h-4" style={{ color: "var(--fb-blue)" }} />
            <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--fb-blue)" }}>
              {answered} de 12 respondidas
            </span>
          </div>
          {cls && (
            <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-pill" style={{ background: cls.bg, color: cls.color, borderRadius: "9999px" }}>
              {cls.label}
            </span>
          )}
        </div>
        <div className="h-1.5 w-full rounded-full" style={{ background: "var(--fb-mid-gray)" }}>
          <div className="h-1.5 rounded-full transition-all duration-300" style={{ width: `${pct}%`, background: "var(--fb-red)" }} />
        </div>
        {answered > 0 && (
          <div className="flex justify-between mt-2">
            <span className="text-[10px]" style={{ color: "var(--fb-slate-gray)" }}>Pontuação parcial: <strong style={{ color: "var(--fb-blue)" }}>{score} pts</strong></span>
            <span className="text-[10px]" style={{ color: "var(--fb-slate-gray)" }}>Máx: 120 pts</span>
          </div>
        )}
      </div>
        ...
        {PERGUNTAS.map((p, idx) => {
          ...
          return (
            <div key={p.field} className="p-4 rounded-md transition-colors"
              style={{ border: `1px solid ${hasErr ? "var(--fb-error)" : val ? "rgba(227,0,15,0.2)" : "var(--fb-mid-gray)"}`, background: val ? "rgba(227,0,15,0.02)" : "#fff" }}>
              <div className="flex items-start gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-sm leading-snug" style={{ color: "var(--fb-dark-gray)" }}>{p.texto} <span style={{ color: "var(--fb-red)" }}>*</span></p>
                </div>
              </div>
                ...
                {OPCOES_AUTOAVALIACAO.map(opcao => {
                  ...
                  return (
                    <button key={opcao} type="button" onClick={() => onChange(p.field, opcao)}
                      ...
                      <span className="block text-sm font-bold">{opcao}</span>
                      <span className="block text-[9px] mt-0.5 uppercase tracking-wide font-semibold opacity-75">{SCORE_LABELS[opcao]}</span>
                    </button>
                  );
                })}
              </div>
