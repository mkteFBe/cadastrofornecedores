...
const inputStyle = { border: "1px solid var(--fb-mid-gray)", borderRadius: "4px", fontSize: "13px", fontFamily: "'AmpleSoft', sans-serif" };
const labelStyle = { fontSize: "10px", fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.05em", color: "var(--fb-blue)", display: "block", marginBottom: "5px" };
...
export function StepISO({ formData, onChange, isoPdfUrl, onIsoPdfChange, errors }: Props) {
  ...
  const extractDates = async (img: string) => {
    ...
    } catch (e) { setExtractionError(`Não foi possível extrair as datas. Preencha manualmente.`); }
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      onIsoPdfChange(publicUrl); toast.success('Certificado enviado!');
      setIsUploading(false);
      await extractDates(img);
    } catch { toast.error('Erro ao enviar certificado.'); setIsoFileName(null); setIsUploading(false); }
  };
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs" style={{ color: "var(--fb-slate-gray)" }}>Norma ABNT ISO 9001:2015 — Sistema de Gestão da Qualidade</p>
      </div>

      <div className="space-y-2">
        <label style={labelStyle}>A empresa possui ISO 9001:2015? <span style={{ color: "var(--fb-red)" }}>*</span></label>
      ...
      {formData.possui_iso_9001 === 'SIM' && (
        <div className="space-y-4 p-4 rounded-md" style={{ background: "var(--fb-light-gray)", border: "1px solid var(--fb-mid-gray)" }}>
          <label style={labelStyle}>Certificado ISO 9001:2015 <span style={{ color: "var(--fb-red)" }}>*</span></label>
          ...
          {isUploading && (
            <div className="flex items-center gap-3 p-3 rounded-md bg-white" style={{ border: "1px solid var(--fb-mid-gray)" }}>
              ...
              <div><p className="text-sm font-semibold" style={{ color: "var(--fb-blue)" }}>Enviando...</p><p className="text-xs" style={{ color: "var(--fb-slate-gray)" }}>{isoFileName}</p></div>
            </div>
          )}
          {!isUploading && isoPdfUrl && (
            <div className="flex items-center gap-3 p-3 rounded-md" style={{ border: "2px solid var(--fb-success)", background: "#F0FBF3" }}>
              <div className="w-8 h-8 rounded flex items-center justify-center flex-shrink-0" style={{ background: "#D4F4DD" }}><FileCheck2 className="w-4 h-4" style={{ color: "var(--fb-success)" }} /></div>
              <div className="flex-1 min-w-0"><p className="text-sm font-bold" style={{ color: "#1A6B30" }}>Certificado enviado</p><p className="text-xs truncate" style={{ color: "#2D8B47" }}>{isoFileName}</p></div>
              <button type="button" onClick={() => { onIsoPdfChange(null); setIsoFileName(null); setExtractionError(null); onChange('iso_data_emissao',''); onChange('iso_data_validade',''); }} className="p-1.5" style={{ color: "#1A6B30" }}><X className="w-4 h-4" /></button>
            </div>
          )}
          {!isUploading && !isoPdfUrl && (
            ...
            <div className="space-y-3 pt-3" style={{ borderTop: "1px solid var(--fb-mid-gray)" }}>
              ...
              {isExtracting && (
                <div className="flex items-center gap-2.5 p-3 rounded-md" style={{ background: "rgba(15,26,44,0.05)", border: "1px solid rgba(15,26,44,0.1)" }}>
                  ...
                  <div className="flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" style={{ color: "var(--fb-blue)" }} /><span className="text-xs font-semibold" style={{ color: "var(--fb-blue)" }}>Lendo certificado e extraindo datas...</span></div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label style={labelStyle}>Data de emissão</label><Input type="date" value={formData.iso_data_emissao} onChange={e => onChange('iso_data_emissao', e.target.value)} style={inputStyle} /></div>
                <div><label style={labelStyle}>Data de validade</label><Input type="date" value={formData.iso_data_validade} onChange={e => onChange('iso_data_validade', e.target.value)} style={inputStyle} /></div>
              </div>
            </div>
          )}
        </div>
      ...
      {formData.possui_iso_9001 === 'NÃO' && (
        <div className="flex items-start gap-3 p-4 rounded-md" style={{ background: "#FFF8E1", border: "1px solid #F0C040" }}>
          ...
          <div>
            ...
            <p className="text-xs leading-relaxed" style={{ color: "#8A6400" }}>Como a empresa não possui ISO 9001, a próxima etapa será uma autoavaliação de maturidade em gestão da qualidade com 12 questões.</p>
          </div>
        </div>
      )}
    </div>
